...
if upload_protocol == "serial":
    def __configure_upload_port(env):
        return [ "-p", env.subst("$UPLOAD_PORT") ]

    env.Replace(
        __configure_upload_port=__configure_upload_port,
        UPLOADER=join(
            '"%s"' % platform.get_package_dir("framework-phy622x") or "",
            "rdwr_phy62x2.py"),
        UPLOADERFLAGS=[
            "-b", env.subst("$UPLOAD_SPEED") or "250000",
            "--startbaud", "115200",
            "-r",
        ],
        UPLOADCMD='$UPLOADER $UPLOADERFLAGS ${__configure_upload_port(__env__)} wh "$SOURCE"'
    )
